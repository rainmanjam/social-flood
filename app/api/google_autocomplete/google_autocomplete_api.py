...
class GoogleAutocompleteParams(BaseModel):
    ...
    client: Optional[ClientType] = Field(None, description="Client identifier (firefox, chrome, safari, opera)")
    ...
    cr: Optional[str] = Field(None, description="Country restrict (e.g., countryUS, countryUK)")

    # Data Source Parameters
    ds: Optional[DataSource] = Field(None, description="Data source for suggestions (yt, i, n, s, v, b, p, etc.)")

    # Search Enhancement Parameters
    spell: Optional[int] = Field(1, description="Enable spell correction (0=disabled, 1=enabled)")
    cp: Optional[int] = Field(None, description="Cursor position in query (character position)")
    gs_rn: Optional[int] = Field(None, description="Request number for sequential numbering")
    gs_id: Optional[str] = Field(None, description="Session ID for tracking")

    # Response Parameters
    callback: Optional[str] = Field(None, description="JSONP callback function name")
    jsonp: Optional[str] = Field(None, description="JSONP wrapper (alternative to callback)")

    # Advanced Parameters
    psi: Optional[int] = Field(None, description="Personalized search (0=disabled, 1=enabled)")
    pq: Optional[str] = Field(None, description="Previous query for query refinement")
    complete: Optional[int] = Field(None, description="Completion type affecting completion logic")
    suggid: Optional[str] = Field(None, description="Suggestion ID for internal tracking")
    gs_l: Optional[str] = Field(None, description="Google search location (internal parameter)")
...
async def get_autocomplete(
    q: str = Query(..., description="Search query string (URL encoded)", example="chrome", title="Query"),
    ...
    hl: str = Query("en", description="Host language using ISO language codes", example="en", title="Host Language"),
...
async def get_suggestions_for_query_async_with_metadata(
    ...
    try:
        ...
        params = {"q": query, "output": output.value, "gl": country, "hl": hl, "spell": spell}
                    ...
                    if start_idx != -1 and end_idx != -1 and end_idx > start_idx:
                        ...
                        if len(data) > 1 and isinstance(data[1], list):
                            if all(isinstance(item, list) and len(item) > 0 for item in data[1]):
                                result["suggestions"] = [
                                    item[0].lower() for item in data[1] if isinstance(item, list) and len(item) > 0
                                ]
                            else:
        ...
        logger.error(
            f"Exception in get_suggestions_for_query_async_with_metadata for query '{query}': {str(e)}", exc_info=True
        )
    ...
    try:
        ...
        if should_try_json_first or looks_like_json or looks_like_jsonp:
            ...
            try:
                        ...
                        if isinstance(data, list):
                            ...
                            if len(data) > 1 and isinstance(data[1], list):
                                ...
                                if not suggestions and len(data) > 4 and isinstance(data[4], dict):
                                    logger.info(
                                        f"No suggestions found for query '{query}', but metadata is available: {json.dumps(data[4])}"
                                    )
                    else:
                ...
                if callback or jsonp:
                    logger.warning(
                        f"JSONP parameters specified but couldn't parse response for query '{query}': callback={callback}, jsonp={jsonp}"
                    )
...
def get_suggestions_for_query(
    q: str, output: OutputFormat = OutputFormat.TOOLBAR, gl: str = "US", hl: str = "en", ds: str = "", spell: int = 1
) -> list:
    """
    ...
    try:
        ...
        should_try_json_first = (
            ...
            and output.lower() in ["chrome", "firefox", "safari", "opera"]
            or isinstance(output, OutputFormat)
            and output in [OutputFormat.CHROME, OutputFormat.FIREFOX, OutputFormat.SAFARI, OutputFormat.OPERA]
        )

        # Check if response looks like JSON (starts with [ or {) or JSONP (contains callback function)
        response_text = (
            response.text if hasattr(response, "text") else response.content.decode("utf-8", errors="ignore")
        )
            ...
            try:
                ...
                if (isinstance(output, str) and output.lower() in ["toolbar", "xml"]) or (
                    ...
                    try:
                        ...
                        logger.error(
                            f"Response Content: {response.text[:200] if hasattr(response, 'text') else 'No text available'}..."
                        )
        else:
            # Try XML parsing first for toolbar/XML output formats
            try:
