...
def format_datetime(dt: Optional[datetime.datetime] = None, format_str: str = "%Y-%m-%d %H:%M:%S") -> str:
    """
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    if logger:
                        logger.warning(
                            f"Retry {attempt + 1}/{max_retries} for {func.__name__} " f"after error: {str(e)}"
                        )
                    ...
                    if logger:
                        logger.error(f"Failed all {max_retries} retries for {func.__name__}: {str(e)}")
