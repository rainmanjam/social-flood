...
def initialize_api_keys():
    ...
    if not _api_keys_set and auth_settings.ENABLE_API_KEY_AUTH:
        logger.warning("No API keys configured. API key authentication is enabled but will reject all requests.")
...
async def authenticate_api_key(
    api_key_header: str = Security(api_key_header), request: Optional[Request] = None
) -> str:
    """
    Validate API key from request header.
