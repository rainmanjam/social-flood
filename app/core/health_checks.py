...
async def check_database_connection() -> Dict[str, Any]:
    ...
    if not settings.DATABASE_URL:
        return {"status": "skipped", "message": "Database URL not configured"}
    ...
    try:
        ...
        if result:
            ...
            raise ServiceUnavailableError(detail="Database query failed", error_type="database_unavailable")
    except ImportError:
        return {"status": "skipped", "message": "Database module not available"}
    except Exception as e:
        logger.exception("Database health check failed")
        raise ServiceUnavailableError(detail=f"Database connection failed: {str(e)}", error_type="database_unavailable")
...
async def check_redis_connection() -> Dict[str, Any]:
    ...
    try:
        ...
        raise ServiceUnavailableError(detail=f"Redis connection failed: {str(e)}", error_type="redis_unavailable")
...
async def check_external_apis() -> Dict[str, Any]:
    ...
    return {
        ...
        "message": (
            "All external APIs are available"
...
async def check_system_resources() -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": "unknown", "message": f"System resources check failed: {str(e)}"}
...
async def check_health(include_details: bool = False, settings: Settings = Depends(get_settings)) -> Dict[str, Any]:
    ...
    results = {
        "database": (
            checks[0] if not isinstance(checks[0], Exception) else {"status": "unhealthy", "message": str(checks[0])}
        ),
        "redis": (
            checks[1] if not isinstance(checks[1], Exception) else {"status": "unhealthy", "message": str(checks[1])}
        ),
        "external_apis": (
            checks[2] if not isinstance(checks[2], Exception) else {"status": "unhealthy", "message": str(checks[2])}
        ),
        "system": (
            checks[3] if not isinstance(checks[3], Exception) else {"status": "unhealthy", "message": str(checks[3])}
        ),
    }
...
async def require_healthy_service(service: str, settings: Settings = Depends(get_settings)) -> None:
    ...
    try:
        ...
        raise ServiceUnavailableError(
            detail=f"Service {service} is unavailable: {str(e)}", error_type=f"{service}_unavailable"
        )
